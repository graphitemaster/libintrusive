...
static avl_node_t*
avl_rotate_ll(avl_node_t* parent, int pfactor, int* cfactor, int* difference)
{
...
static avl_node_t*
avl_rotate_rr(avl_node_t* parent, int pfactor, int* cfactor, int* difference)
{
...
static inline avl_node_t* avl_rotate_lr(avl_node_t* parent, int pfactor)
{
    ...
    if (child->right)
    {
        ...
        parent->left = avl_rotate_rr(
            child, avl_get_factor(child), &cfactor, &difference);
    }
    else
    {
    ...
    avl_node_t* node =
        avl_rotate_ll(parent, pfactor - difference, &cfactor, NULL);
...
static inline avl_node_t* avl_rotate_rl(avl_node_t* parent, int pfactor)
{
    ...
    avl_node_t* node =
        avl_rotate_rr(parent, pfactor + difference, &cfactor, NULL);
    avl_set_factor(node, cfactor);
    return node;
}
...
avl_node_t*
avl_search(avl_tree_t* tree, avl_node_t* node, avl_compare_t compare)
{
